...
//
// CTA の type は A8 提携状態に連動する:
//   "official"  = 素の公式サイトURL（バッジ = 広告ではありません / rel = nofollow）
//   "a8-active" = A8 アフィリリンク（バッジ = PR / rel = nofollow sponsored / ページ上部に「PR — 広告を含む」）
// type と url は必ずセットで変えること。片方だけ変えると表示と実態が食い違い、ステマ規制上まずい。
//
// A8 提携状況（2026-07-20 時点）:
//   松井証券   = 提携中（2026-07-15 承認）→ a8-active。素材ID:028 テキストリンク。
//   DMM 株     = 提携中（2026-07-14 承認）→ a8-active。テキストリンク（DMM 株の魅力訴求）。
//   GMOクリック / eスマート = 審査中（2026-07-14 申込）→ official のまま。承認され次第 a8-active に。
//   SBI / 楽天 = A8 に本体プログラム無し → 恒久的に official（この2社が最頻出の着地先＝無報酬でも正直に上位表示する設計の要）。
// ⚠ 金融庁「広告表示内容の基準例」順守: メリットだけでなくリスク・デメリット（unfit）を必ず併記。
//    「儲かる」「負けない」等の断定・誇大表現は禁止（DMM ガイドライン明記）。「DMM 株」はスペース必須。
...
export const COMPANIES = {
  ...
  MATSUI: {
    ...
    cta: {
      // A8 提携中（2026-07-15 承認・素材ID:028 テキストリンク）。成果条件: 広告主新規・WEB申込後30日以内の口座開設完了。
      type: "a8-active",
      label: "公式サイトで詳細を見る",
      url: "https://px.a8.net/svt/ejp?a8mat=4B8094+9FD54Y+3XCC+64C3M",
    },
  },
  AU: {
    name: "三菱UFJ eスマート証券",
  ...
  DMM: {
    ...
    cta: {
      // A8 提携中（2026-07-14 承認・テキストリンク）。成果条件: 広告主新規・WEBアカウント登録後60日以内に1回の取引。
      type: "a8-active",
      label: "公式サイトで詳細を見る",
      url: "https://px.a8.net/svt/ejp?a8mat=4B8094+9SGOG2+1WP2+15ORS2",
    },
  },
};
